...
function codexHomeDir(env?: Record<string, string>): string {
  const fromRuntimeEnv = env?.CODEX_HOME ?? env?.AUTOPILOT_CODEX_HOME;
  if (typeof fromRuntimeEnv === "string" && fromRuntimeEnv.trim().length > 0) {
    return fromRuntimeEnv.trim();
  }
  const fromEnv = process.env.CODEX_HOME ?? process.env.AUTOPILOT_CODEX_HOME;
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  if (preparedAuth.check.level === "error") {
    throw new Error(preparedAuth.check.message);
  }

  await ensureCodexSkillsInjected(onLog, codexHomeDir(preparedAuth.env));

  const billingType = preparedAuth.billingType;
  const runtimeEnv = ensurePathInEnv({ ...process.env, ...preparedAuth.env });
  ...
  const combinedCommandNotes = [...preparedAuth.commandNotes, ...commandNotes];
  ...
  const runAttempt = async (resumeSessionId: string | null) => {
    ...
    if (onMeta) {
      await onMeta({
        ...
        command,
        cwd,
        commandNotes: combinedCommandNotes,
        commandArgs: args.map((value, idx) => {
          if (idx === args.length - 1 && value !== "-") return `<prompt ${prompt.length} chars>`;
          return value;
        }),
        env: redactEnvForLogs(preparedAuth.env),
        prompt,
        context,
      });
    }
